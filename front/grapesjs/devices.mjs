  ...
  <path d="M2.5 13.5A.5.5 0 0 1 3 13h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5M13.991 3l.024.001a1.5 1.5 0 0 1 .538.143.76.76 0 0 1 .302.254c.067.1.145.277.145.602v5.991l-.001.024a1.5 1.5 0 0 1-.143.538.76.76 0 0 1-.254.302c-.1.067-.277.145-.602.145H2.009l-.024-.001a1.5 1.5 0 0 1-.538-.143.76.76 0 0 1-.302-.254C1.078 10.502 1 10.325 1 10V4.009l.001-.024a1.5 1.5 0 0 1 .143-.538.76.76 0 0 1 .254-.302C1.498 3.078 1.675 3 2 3zM14 2H2C0 2 0 4 0 4v6c0 2 2 2 2 2h12c2 0 2-2 2-2V4c0-2-2-2-2-2"/>
    ...
    editor.on("update", ()=>{
        ...
        if(editor.getDevice() === "desktop"){
            //device was loaded when hidden, we should recompute
            loadDevice('xxl');
        }
    })

    function loadDevice(code){
        let deviceChosen = editor.DeviceManager.getDevices().find(d=>d.id === code) ;
        let deviceWidth = Number(deviceChosen.attributes.width.replace("px", ""));
        const screenWidth = editor.Canvas.getElement().offsetWidth;
        if(screenWidth === 0){ 
            console.log("editor not displayed, ignore")
            return; 
        }
        const zoomScale = (screenWidth / deviceWidth) * 100;
        // Clamp the zoom scale between 10% and 100% to avoid extreme scaling
        const clampedZoomScale = Math.min(Math.max(zoomScale, 10), 100);

        console.log("screenWidth", screenWidth, "zoomScale",zoomScale , "clampedZoomScale", clampedZoomScale) ;
        if(clampedZoomScale < 100){
            // Calculate the scaled height
            deviceChosen.attributes.height =  editor.Canvas.getElement().offsetHeight*(1/(zoomScale/100))
            console.log("computed height", deviceChosen.attributes.height) ;
        }

        editor.Canvas.setZoom(clampedZoomScale);
        editor.Canvas.setCoords(0,0, {toWorld: true});
        editor.setDevice(code);
